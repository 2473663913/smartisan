                        ...
                        <div class="address-common-table js-multiple-address-panel">
                            <ul class="address-item-list clear js-address-item-list">
                                <li class="js-choose-address  " :class="{'selected-address-item':receiveIndex===index}" v-for="(receive,index) in receiveInfo">
                                    <div class="address-item" @click="chooseReceive(index)">
                    ...
                    <div class="invoice-detail"> 发票抬头： 
                        <div class="radio-box"> 
                            <label> 
                                ...
                                <span class="blue-radio " :class="{'blue-radio-on':porcom.person}" @click="checkdeCorcom(true)"><a></a></span>  个人
                            </label> 
                            <label> 
                                <input type="radio" class="hide"> 
                                <span class="blue-radio" :class="{'blue-radio-on':!porcom.person}" @click="checkdeCorcom(false)"><a></a></span>  单位
                            </label> 
                        </div> 
                        <div class="module-form-row form-item fn-hide js-invoice-title" v-if="!porcom.person"> 
                            <div class="module-form-item-wrapper no-icon small-item"> 
                                <i v-show="!porcom.name">请填写公司发票抬头</i> 
                                <input type="text" class="js-verify" v-model="porcom.name"> 
                                <div class="verify-error" v-show="!porcom.name">必填</div>
                            </div> 
                        </div> 
                    </div> 
            ...
            <div class="gray-box">
                ...
                <div class="box-inner ui-goods-cart">
                    ...
                    <div class="cart-table">
                        <div class="cart-group js-cart-group" v-for="(item,index) in checkdeGoods">
                ...
                <div class="box-inner"> 
                    <div class="order-discount-line"> 
                        <p> 商品总计： <span>¥ {{getCounts}}.00</span> </p> 
                ...
                <div class="box-inner"> 
                    <div class="last-payment clear"> 
                        <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="submitOrderHandler"> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{getPrice+freight}}.00</em></span> 
                    </div> 
                </div>
            </div>
        </div>
        <adressPop v-if="popShow" @close="closePop"></adressPop>
    </div>
</template>

<script>
import adressPop from '@/components/adress-pop'
export default {
    data(){
        return {
            receiveIndex:0,
    ...
    //生命周期钩子函数，在模板渲染成html前调用，即通常初始化某些属性值，然后再渲染成视图。
    created(){
    ...
    computed:{
        //接收vuex中计算的选中商品
        checkdeGoods(){
            return this.$store.getters.checkdeGoods
        },
        //接收vuex中计算的选中商品数量
        getCounts(){
            return this.$store.getters.allCheckdeGoods
        },
        //接收vuex中计算的商品总价
        getPrice(){
            return this.$store.getters.allCheckdeGoodsPrice
        },
        //计算运费
        freight(){
            let freight=8
    ...
    methods:{
        ...
        closePop(){
            ...
            this.$store.state.receiveInfo.forEach((receive,index)=>{
                if(receive.default){
                    this.receiveIndex=receive.index
                    return
                }
            })
        },
        ...
        //发票类型选择
        checkdeCorcom(boole){
            this.porcom.person=boole
        },
        //提交订单
        submitOrderHandler(){
            //发票信息为必填，先判断是公司还是个人，是公司就return
            if(!this.porcom.person && !this.porcom.name) return
            //所选中的地址
            let receiveinfo=this.receiveInfo[this.receiveIndex]
            if(this.porcom.person){
                this.porcom.name='个人'
            }
            let iDate=new Date()
            ...
            let data={
                ...
                goodsData:this.checkdeGoods,
                price:this.getPrice,
                freight:this.freight,
                porcomName:this.porcom.name,
                date:iDate.getFullYear()+'-'+month+'-'+day,
                inforeceive:receiveinfo,
                isPay:false
            }
            //向vuex中发送data
