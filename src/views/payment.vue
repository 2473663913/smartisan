        ...
        <div class="content page-order-payment">
            <div class="gray-box clear">
                ...
                <div class="box-inner payment-checkout-panel clear">
                    <span class="jianguo-blue-main-btn big-main-btn js-payment-order" @click="orderPayHander(getOrder.orderId)" v-if="!getOrder.isPay">
                        <a>现在支付</a>
                    </span>
                    <span class="is-pay" v-else>已完成</span>
                    <span class="prices"> 应付金额：   <em><span>¥ </span>{{getOrder.price+getOrder.freight}}.00</em>   </span>
                </div>
            </div>
            <div class="confirm-detail">
                <div class="info-title">订单编号</div>
                <p class="info-detail">{{getOrder.orderId}}</p>
            </div>
            <div class="confirm-detail">
                <div class="info-title">收货信息</div>
                <p class="info-detail">姓名：{{getOrder.inforeceive.name}}</p>
                <p class="info-detail">联系电话：{{getOrder.inforeceive.phone}}</p>
                <p class="info-detail">详细地址：{{getOrder.inforeceive.province}} {{getOrder.inforeceive.city}} {{getOrder.inforeceive.county}} {{getOrder.inforeceive.add}}</p>
            </div>
            <div class="confirm-detail">
                <div class="info-title">发票信息</div>
                <p class="info-detail">发票类型：电子发票</p>
                <p class="info-detail">发票抬头：{{getOrder.porcomName}}</p>
            ...
            <div class="confirm-goods-table">
                <div class="cart-items clear" v-for="(good,index) in getOrder.goodsData">
                    <div class="name fn-left hide-row">
                        <div class="name-cell"> 
                            <a href="http://www.smartisan.com/shop/#/item/100029901" :title="good.title+'('+good.spec_json.show_name+')'" target="_blank">{{good.title}}（{{good.spec_json.show_name}}）</a><br>
                        </div>
                    </div>
                    <div class="subtotal fn-right">
                        <div class="subtotal-cell">¥ {{good.price*good.count}}.00</div>
                    </div>
                    <div class="goods-num fn-right">1</div>
                    <div class="price fn-right">¥ {{good.price}}.00</div> 
                </div>
                
            </div>
            <div class="order-discount-line">
                <p> 商品总计： <span>¥ {{getOrder.price}}.00</span> </p>
                <p> 运费： <span>+ ¥ {{getOrder.freight}}.00</span> </p>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
    orderPayHander(id){
            this.$store.commit('payHander',id)
            alert('成功支付'+(this.getOrder.price+this.getOrder.freight)+'元')
